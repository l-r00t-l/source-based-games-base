...
{
public:
	GET_NETVAR(get_flags, "CBasePlayer->m_fFlags", int32_t);
// 	GET_NETVAR(int, get_flags, _("DT_BasePlayer"), _("m_fFlags"));
	GET_NETVAR(get_health, "CBasePlayer->m_iHealth", int);
// 	GET_NETVAR(int, get_health, _("DT_BasePlayer"), _("m_iHealth"));
	GET_NETVAR(get_sequence, "CBasePlayer->m_nSequence", int);
// 	GET_NETVAR(int, get_sequence, _("DT_BasePlayer"), _("m_nSequence"));
	GET_NETVAR(get_tick_base, "CBasePlayer->m_nTickBase", int);
// 	GET_NETVAR(int, get_tick_base, _("DT_BasePlayer"), _("m_nTickBase"));
	GET_NETVAR(get_impulse, "CBasePlayer->m_nImpulse", int);
// 	GET_NETVAR(int, get_impulse, _("DT_CSPlayer"), _("m_nImpulse"));
	GET_NETVAR(get_vehicle, "CBasePlayer->m_hVehicle", c_handle<c_base_player>);
// 	GET_NETVAR(c_handle < c_base_player >, get_vehicle, _("DT_BasePlayer"), _("m_hVehicle"));
	GET_NETVAR(get_fall_velocity, "CBasePlayer->m_flFallVelocity", float);
...
// 	GET_NETVAR(vec3_t, get_origin, _("DT_BaseEntity"), _("m_vecOrigin"));
	GET_NETVAR(get_life_state, "CBasePlayer->m_lifeState", int);
// 	GET_NETVAR(int, get_life_state, _("DT_BasePlayer"), _("m_lifeState"));
	GET_NETVAR(has_heavy_armor, "CBasePlayer->m_bHasHeavyArmor", bool);
// 	GET_NETVAR(bool, has_heavy_armor, _("DT_CSPlayer"), _("m_bHasHeavyArmor"));
	GET_NETVAR(has_helmet, "CBasePlayer->m_bHasHelmet", bool);
// 	GET_NETVAR(bool, has_helmet, _("DT_CSPlayer"), _("m_bHasHelmet"));
	GET_NETVAR(get_armour_value, "CBasePlayer->m_ArmorValue", int);
// 	GET_NETVAR(int, get_armour_value, _("DT_CSPlayer"), _("m_ArmorValue"));
	GET_NETVAR(get_aim_punch_angle, "CBasePlayer->m_aimPunchAngle", qangle_t);
// 	GET_NETVAR(qangle_t, get_aim_punch_angle, _("DT_BasePlayer"), _("m_aimPunchAngle"));
	GET_NETVAR(get_active_weapon_handle, "CBasePlayer->m_hActiveWeapon", c_handle<c_weapon>);
...
// 	GET_VFUNC(const char* (__thiscall*)(void*), get_classname(), 59);
	GET_VFUNC(get_classname, 59, const char*);
...
// 	GET_VFUNC(void(__thiscall*)(void*), update_client_side_animations(), 224);
	GET_VFUNC(update_collistion_bounds, 340, void);
	GET_VFUNC(studio_frame_advance, 220, void);
	...
	void invalidate_physics_recursive(const int flag)
	{
		...
		static auto invalidate_physics_recursive_fn = reinterpret_cast<void(__thiscall*)(void*, int)>(g_utils.find_sig(g_sdk.m_modules.m_client_dll, "55 8B EC 83 E4 F8 83 EC 0C 53 8B 5D 08 8B C3"));
		return invalidate_physics_recursive_fn(this, flag);
	}

	void select_item(const char* string, const int sub_type)
	{
		static auto select_item_fn = reinterpret_cast <void(__thiscall*)(void*, const char*, int)> (g_utils.find_sig(g_sdk.m_modules.m_client_dll, "55 8B EC 56 8B F1 ? ? ? 85 C9 74 71 8B 06"));
		return select_item_fn(this, string, sub_type);
	}

	bool using_standard_weapons_in_vehicle()
	{
		static auto sig_fn = reinterpret_cast <bool(__thiscall*)(void*)> (g_utils.find_sig(g_sdk.m_modules.m_client_dll, "56 57 8B F9 8B 97 ? ? ? ? 83 FA FF 74 41"));
		return sig_fn(this);
	}

	bool post_think_v_physics()
	{
		static auto sig_fn = reinterpret_cast <bool(__thiscall*)(void*)> (g_utils.find_sig(g_sdk.m_modules.m_client_dll, "55 8B EC 83 E4 F8 81 EC ? ? ? ? 53 8B D9 56 57 83 BB"));
		return sig_fn(this);
	}

	void physics_simulated_entites()
	{
		static auto sig_fn = reinterpret_cast <void(__thiscall*)(void*)> (g_utils.find_sig(g_sdk.m_modules.m_client_dll, "56 8B F1 57 8B BE ? ? ? ? 83 EF 01 78 74"));
		return sig_fn(this);
	}

	bool physics_run_think(const int index)
	{
		static auto physics_run_think_fn = reinterpret_cast <bool(__thiscall*)(void*, int)> (g_utils.find_sig(g_sdk.m_modules.m_client_dll, "55 8B EC 83 EC 10 53 56 57 8B F9 8B 87"));
		return physics_run_think_fn(this, index);
	}

	void post_think()
	{
		g_sdk.m_interfaces.m_mdl_cache->begin_lock();
		{
			if (this->is_alive())
			{
		...
		g_sdk.m_interfaces.m_mdl_cache->end_lock();
	}
	...
	vec3_t get_shoot_pos()
	{
		...
		*/
		...
		static auto eye_pos_fn = reinterpret_cast<float* (__thiscall*)(void*, vec3_t*)>(g_utils.find_sig(g_sdk.m_modules.m_client_dll, "55 8B EC 56 8B 75 08 57 8B F9 56 8B 07 FF 90 ? ? ? ?")); // 8B 07 FF 90 ? ? ? ? 80 BF ? ? ? ? ? 74 10 
		vec3_t eye_pos;
		eye_pos_fn(this, &eye_pos);
		return eye_pos;
	}

	[[nodiscard]] c_weapon* get_active_weapon()
	{
	...
	bool is_breakable()
	{
		...
		*/
		...
		if (this->get_collideable()->get_collision_group() != collision_group_pushaway && this->get_collideable()->get_collision_group() != collision_group_breakable_glass
			&& this->get_collideable()->get_collision_group() != collision_group_none)
			return false;
		...
		else
		{
			...
			if (strcmp(class_name, "func_breakable") == 0 || strcmp(class_name, "func_breakable_surf") == 0)
			{
				if (strcmp(class_name, "func_breakable_surf") == 0)
				{
