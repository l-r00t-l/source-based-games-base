...
const uint32_t c_utils::compile_time(const char* data, const uint32_t value /*= BASE*/)
{
	return (data[0] == '\0') ? value : compile_time(&data[1], (value ^ uint32_t(data[0])) * PRIME);
}

const uint32_t c_utils::run_time(std::string_view data)
{
	int32_t hashed = BASE;
