...
void c_misc::edge_jump_pre_prediction(c_user_cmd* cmd)
{
	...
	c_misc::get()->flags_backup = g_sdk.m_local->get_flags();
}

void c_misc::edge_jump_post_prediction(c_user_cmd* cmd)
{
	if (!g_utils.is_bind_active(key_bind_t{ PRESSED, VK_XBUTTON1 }))
		return;
	...
	if (c_misc::get()->flags_backup & fl_on_ground && !(g_sdk.m_local->get_flags() & fl_on_ground))
		cmd->m_buttons |= in_jump;
